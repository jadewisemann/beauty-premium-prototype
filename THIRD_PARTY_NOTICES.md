...
`@mediapipe/tasks-vision` 1.0.1, the copied WASM loaders/binaries under `public/vendor/mediapipe/`, and the Hair Segmenter model under `public/models/` are bundled for local browser execution. Exact model URL and hash are recorded in `public/models/models.manifest.json`.
...
The manifest records the official Hair Segmenter model-card URL as licensing evidence.
...
라이브 파운데이션·립·아이섀도·아이라인·블러셔 렌더링에 OpenMakeupSDK 0.1.0을 사용한다. 얼굴 변형 기능은 사용하지 않는다. 메이크업 셰이더·모델·패턴은 고정된 jsDelivr 0.1.0 경로에서 로드한다.
